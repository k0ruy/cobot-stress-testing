...
def create_individual_questionnaires():
    """
    Create individual questionnaires for each patient.
    :return: None. Saves the questionnaires in a csv file.
    """
        ...
        for folder in SAVED_DATA.iterdir():
            if folder.is_dir():
                ...
                for task in ['cobot', 'manual', 'rest', 'stroopeasy', 'stroophard']:
                    ...
                    q = questionnaires[questionnaires['ID'] == int(patient_id)]
                    ...
                    q.to_csv(folder / f'questionnaire_{task}_{int(patient_id)}.csv', index=False)
    ...
    for task in ['cobot', 'manual']:
        for folder in Path(EXPERIMENTS, task).iterdir():
            if folder.is_dir():
                ...
                q = pd.read_csv(SAVED_DATA / f'patient-{patient_id}' / f'questionnaire_{task}_{int(patient_id)}.csv')
                ...
                q.to_csv(SAVED_DATA / f'patient-{patient_id}' / f'questionnaire_{task}_{int(patient_id)}.csv',
                         index=False)
    ...
    for folder in SAVED_DATA.iterdir():
        if folder.is_dir():
            ...
            for task in ['cobot', 'manual']:
                ...
                ecg_features = pd.read_csv(folder / f'ecg_{task}_{int(patient_id)}.csv')
                # get the EDA features:
                eda_features = pd.read_csv(folder / f'eda_{task}_{int(patient_id)}.csv')
                # get the EMG features:
                emg_features = pd.read_csv(folder / f'emg_{task}_{int(patient_id)}.csv')

                # check if the files already have the Stress column, if so skip:
                if 'Stress' in ecg_features.columns:
                    continue

                # get the questionnaires:
                questionnaires = pd.read_csv(folder / f'questionnaire_{task}_{int(patient_id)}.csv')
                ...
                ecg_features.to_csv(folder / f'ecg_{task}_{int(patient_id)}.csv', index=False)
                eda_features.to_csv(folder / f'eda_{task}_{int(patient_id)}.csv', index=False)
                emg_features.to_csv(folder / f'emg_{task}_{int(patient_id)}.csv', index=False)
...
def main() -> None:
    ...
    # create the individual questionnaires for each patient:
    create_individual_questionnaires()
